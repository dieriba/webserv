...
/*   Updated: 2023/09/19 23:02:39 by dtoure           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	Parser :: checkIndent(std :: string &line, const int indent, const size_t nbline)
{
	...
	if (index != indent || s_it == line.end())
		notifyError("Bad Indentation at line " + LibHelp :: toString(nbline) + ".");
	if (isspace(*(line.begin() + index)))
		notifyError("Only tabulations are accepted as indentation, at line " + LibHelp :: toString(nbline) + ".");
	return _error.empty();
}
...
bool	Parser :: parse(const std :: string &filename, HttpServer &httpServ)
{
	...
	if (lines.empty() || bracket(content.begin(), content.end(), 0) == content.end()) {
		notifyError("Bracket not closed");
		return false;
	}
	...
	for (vstring_it it = lines.begin(); it != lines.end(); it++)
	{
		...
		if (LibHelp :: find(it -> begin(), it -> end(), '{')) {
			...
			if ((blockName = getBlockName(it -> begin(), it -> end())) == "") {
				notifyError("No spaces/chars accepted after braces");
				return false;
			}
			...
			if (blockName == "server")
				first_server++;
			//if (it != lines.begin())
			//	notifyError("Server block must be at the top of file");
			if (blockName != "server") {
				if (tmp.size() <= 0 || tmp.size() > 2)
					notifyError("Wrong args at: " + blockName);
				else if (tmp.at(0) != "location")				
					notifyError("Wrong block name: " + blockName);
			}
			...
			if (blockName == "/" && first_server >= 1) {
				...
				std::cout << "Server cree " << server << std::endl;
