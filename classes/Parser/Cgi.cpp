...
/*   Updated: 2023/09/20 02:01:09 by dtoure           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Cgi::setPid(const pid_t& pid)
{
	_pid = pid;
}
...
int	Cgi::resetCgi(const int& epfd)
{
	...
	_response.clear();
	_req->setIsCgi(false);
	std::cout << "Client Socket value : " << _client -> getSocketFd() << std::endl;
	...
	std::cout << "ResetCgi " << std::endl;
	return 0;
}

int		Cgi::handleRequest(int epfd, epoll_event& /* ev */)
{
	std::cout << "Entered handle cgi" << std::endl;
