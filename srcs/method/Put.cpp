...
Put::Put():_request_body_size(0){};
Put::Put(const Put& rhs):Method(rhs),_request_body_size(rhs._request_body_size){};
...
/*----------------------------------------GETTER----------------------------------------*/
const size_t& Put::getRequestBodySize(void) const {return _request_body_size;}
/*----------------------------------------GETTER----------------------------------------*/


/*----------------------------------------SETTER----------------------------------------*/
void Put::updateSize(const size_t& size)
{
...
/*----------------------------------------MEMBER FUNCTION----------------------------------------*/
void Put::clearRequestBodySize(void)
{
    _request_body_size = 0;
}

int Put::writeToFileMutltipartData(HttpRequest& req, const size_t& size)
{
...
int Put::writeToFile(HttpRequest& req, const size_t& pos, const size_t& bytes)
{
    try
    {
...
int Put::writeToFile(HttpRequest& req)
{
    try
    {
        std::string& alias = req.getBuffer();
...
int Put::handleMultipartData(ClientSocketStream& client, HttpRequest& req)
{
